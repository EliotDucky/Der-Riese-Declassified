...
function init()
{
	...
	if( cymbal_monkey_exists( level.w_cymbal_monkey_upgraded) )
	{
		...
		level.cymbal_monkey_upgrade_model = UPGRADE_WM;
	}
...
//Call on furnace trig
function furnaceCry(){
	//Monkey scream
	self PlaySound(FURNACE_NON_QUEST_MNK_SCREAM_VOX);
	wait(8);
	//wait(SoundGetPlaybackTime(FURNACE_NON_QUEST_MNK_SCREAM_VOX) / 1000);
	//Sam says come back later
	self PlaySound(FURNACE_NON_QUEST_SAM_ALIAS);
}
...
function player_throw_cymbal_monkey(grenade,num_attractors,max_attract_dist,attract_dist_diff)
{
	...
	if( IsDefined( grenade ) )
	{
		...
		model_name = level.cymbal_monkey_model;
		if(isdefined(level.weaponZMCymbalMonkey) && self zm_weapons::has_upgrade(level.weaponZMCymbalMonkey)){
			model_name = level.cymbal_monkey_upgrade_model;
		}
		...
		if( isDefined( grenade ) )
		{
			...
			in_furnace = grenade IsTouching(furnace_trig);
			if(valid_poi)
			{
				...
				if(!(isdefined(level.mnk_can_furnace) && level.mnk_can_furnace)){
					furnace_trig thread furnaceCry();
				}
			}
			else
			{
